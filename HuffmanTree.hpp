...
int main()
{
    ...
    std::cout << "请输入权值的个数：" << std::endl;
    std::cin >> n;
    HuffmanTree HT(n);
    HuffmanCode* HC = new HuffmanCode[n];
    int* weight = new int[n];// 动态生成存放n个权值的空间
    if (weight == nullptr) {
    ...
    std::cout << "请依次输入" << n << "个字母：" << std::endl;
    for (i = 0; i < n; i++) {
        std::cin >> sign[i];
    }
    std::cout << "请依次输入" << n << "个权值(整型)：" << std::endl;
    for (i = 0; i < n; i++) {
        std::cin >> weight[i]; // 依次输入权值
    }
    HT.HuffmanTreeing(weight, sign);
    HT.HuffmanCoding(HC); // n个哈夫曼编码存于HC
    std::string Input;

    std::cout << "输入你要编码的字符串" << std::endl;
    std::cin >> Input;
    std::string OutCodes = HT.StringToCodes(HC, Input);
    std::cout << "输出字符串的编码" << std::endl;
    std::cout << OutCodes << std::endl;
    std::string OutString = HT.CodesToString(OutCodes);
    std::cout << "输出编码解释后的字符串" << std::endl;
